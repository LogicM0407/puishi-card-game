...
const CHANGELOG = Object.freeze([
  { version: "v2.4.1", items: [
    "重构：一切未竟改为生成历史最高卡色牌并强制立刻打出（替代原简化重放）；无法打出时保留在手牌",
    "完整实装：七迹实时选牌流程（按总分排序动态选牌窗口、多轮循环分配、15秒超时自动分配、手牌满跳过、AI 自动选牌）",
    "精确化：乌托邦序曲减分/弃牌响应改为精确抵消（减分直接还原、弃牌按 uid 精确还回）",
    "修复：七迹选牌阶段机器人未被调度，导致延迟并误报「操作超时自动分配」",
    "新增：世界树生机在玩家状态栏显示进度；完整记录对局操作日志并提供「复制日志」按钮用于 bug 复盘"
  ]},
